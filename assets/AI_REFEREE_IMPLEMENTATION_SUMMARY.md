...
### V4.3 - 回测生成器优化（2025-01-29）

🎯 **三个致命问题修复**：

1. **修复数据穿越**：正确获取未来数据计算标签
   - 修复前：`get_stock_data(..., end_date=date)` 只能获取历史数据
   - 修复后：新增 `_get_future_data()` 方法，正确获取未来数据
   - 影响：避免标签全部为0，确保训练数据有效

2. **动态止盈止损**：使用动态判断而非傻傻持有
   - 修复前：持有5天后判断盈亏
   - 修复后：逐日检查，触发止盈/止损立即平仓
   - 更符合实盘逻辑，减少回撤

3. **样本分布一致**：复刻真实策略逻辑
   - 修复前：只选"涨幅>5%"的股票，样本单一
   - 修复后：支持放量、缩量、梯量三种形态
   - 训练集分布 = 实盘分布，模型有实用价值

详细说明：见 [BacktestGenerator 优化文档](BACKTEST_GENERATOR_OPTIMIZATION.md)
