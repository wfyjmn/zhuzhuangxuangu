...
**核心改进**：
1. 新增 `train_time_series()` 方法：多折时序交叉验证（比单次切分更稳健）
2. 优化默认参数：n_estimators=200, learning_rate=0.05（更保守，避免过拟合）
3. 改进代码结构：新增 `_get_default_params()` 和 `_get_model_instance()` 方法
4. 增强测试覆盖：新增 `train_time_series` 测试和对比测试
