...
综合优化版模型训练脚本
集成所有优化措施：
1. 增强版特征工程（主力资金、北向资金等）
2. 目标调整：5-6%涨幅，7-10天预测窗口
3. 数据增强：200只股票，5年历史数据
4. 模型调优：优化阈值和样本权重
5. 防止数据泄露
"""
...
class OptimizedTrainer:
    ...
    def _load_config(self) -> Dict:
        ...
        config = {
            "data": {
                ...
                "min_return_threshold": 0.05,  # 降低至5%
                "max_return_threshold": 0.06,  # 上限6%
                "prediction_days": [7, 8, 9, 10],  # 7-10天
                "n_stocks": 200  # 200只股票
            },
            "model": {
                "learning_rate": 0.01,
                ...
                "reg_alpha": 1,
                "gamma": 1,
                "scale_pos_weight": 3,  # 增加正样本权重（目标5-6%更难）
                "n_estimators": 800,
                "early_stopping_rounds": 50,
                "random_state": 42
            },
            "threshold": {
                "target_precision": 0.65,  # 目标精确率65%
                "default_threshold": 0.4,  # 降低默认阈值
    ...
    def collect_all_data(self, stock_codes: List[str],
        ...
        print(f"时间范围: {start_date} 至 {end_date} (5年)")
        print("=" * 70)
        ...
        for i, stock_code in enumerate(stock_codes, 1):
            ...
            if df is not None and len(df) > 200:  # 至少200条数据（5年约1200天）
                all_data.append(df)
                success_count += 1
                print(f"✓ ({len(df)}条)")
            else:
    ...
    def create_labels_separately(self, train_df: pd.DataFrame,
        ...
        """
        分别创建标签（优化目标：5-6%，7-10天）
        """
        print("=" * 70)
        print("【步骤5】创建标签（目标：5-6%涨幅，7-10天）")
        print("=" * 70)

        min_return = self.config['data']['min_return_threshold']
        max_return = self.config['data']['max_return_threshold']
        ...
        for name, df in [('训练集', train_df), ('验证集', val_df), ('测试集', test_df)]:
            ...
            print(f"{name}:")
            print(f"  样本数: {len(df)}")
            print(f"  正样本（{min_return*100:.0f}%-{max_return*100:.0f}%）: {df['label'].sum()} ({df['label'].mean():.2%})")
    ...
    def optimize_threshold(self, y_val: np.ndarray, y_val_pred_proba: np.ndarray) -> float:
        ...
        for threshold in thresholds:
            ...
            if y_pred.sum() > 0:
                ...
                # 打印部分阈值
                if abs(threshold - 0.4) < 0.02 or abs(threshold - best_threshold) < 0.01:
    ...
    def evaluate_model(self, X_test: np.ndarray, y_test: np.ndarray,
        ...
        if metrics['precision'] >= 0.75:
            ...
            print(f"  ❌ 不足：精确率 {metrics['precision']:.2%}")
        ...
        if metrics['auc'] >= 0.75:
            ...
            print(f"  ⚠️ 尚可：AUC {metrics['auc']:.4f}")
        else:
            print(f"  ❌ 不足：AUC {metrics['auc']:.4f}")
        ...
        print(f"  预测为负的样本数: {(1-y_pred).sum()} ({(1-y_pred).sum()/len(y_pred)*100:.2f}%)")
        print()
    ...
    def save_model(self, threshold: float, metrics: Dict):
        ...
        # 保存模型
        model_path = os.path.join(model_dir, "optimized_model.pkl")
        with open(model_path, 'wb') as f:
            pickle.dump(self.model, f)
        print(f"✓ 模型已保存: {model_path}")

        # 保存特征名称
        feature_path = os.path.join(model_dir, "optimized_features.pkl")
        ...
        metadata = {
            ...
            'data_description': '优化版本：200股×5年，目标5-6%，7-10天',
        ...
        metadata_path = os.path.join(model_dir, "optimized_metadata.json")
    ...
    def run(self):
        ...
        print("综合优化版模型训练")
        print("优化措施：")
        print("  1. 增强版特征工程（主力资金、北向资金等）")
        print("  2. 目标调整：5-6%涨幅，7-10天预测窗口")
        print("  3. 数据增强：200只股票，5年历史数据")
        print("  4. 模型调优：优化阈值和样本权重")
        print("  5. 防止数据泄露")
        print("=" * 70 + "\n")
        ...
        print("✓ 训练完成！")
        print("=" * 70)
        print(f"\n模型文件: assets/models/optimized_model.pkl")
